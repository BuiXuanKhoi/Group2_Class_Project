    ...
    fun loginSuccess(){
        viewlog.right.observe(viewLifecycleOwner){success->
            if(success){
                ...
                state.saveLoginInfo(emailLog,passLog,requireContext(),true) // Vì chưa chắc sẽ dùng tài khoản Sign Up để đăng nhập, cho nên những thông tin đăng nhập của User sẽ được set khi đăng nhập
