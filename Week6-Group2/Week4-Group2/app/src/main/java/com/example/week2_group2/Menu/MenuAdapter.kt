...
class MenuAdapter : ListAdapter<Info,MenuAdapter.MenuViewHolder>(MenuDiffUtil()) {
    ...
    class MenuViewHolder(itemView: View,listener: onItemClickListener) : RecyclerView.ViewHolder(itemView) {
        ...
        fun seeData(info: Info){
            ...
            val imageOfRes = itemView.findViewById<ImageView>(R.id.avt_res)
...
// Khi xảy ra sự kiện itemview bị nhấn trúng trong viewholder, sẽ gọi đến phương thức onItemClick của instance listener : listener.onItemClick(adapterPosition) với adapterPosition là vị trí hiện tại

// Hàm onItemClick tồn tại để truyền tham số position qua bên main screen

// Cho mListener = listener trong setOnItemClickListener để lấy case