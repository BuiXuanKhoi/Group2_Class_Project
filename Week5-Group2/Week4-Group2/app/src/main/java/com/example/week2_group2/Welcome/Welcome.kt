...
class Welcome : Fragment() {
    ...
    override fun onViewCreated(view: View, savedInstanceState: Bundle?) {
        ...
        welcomeBind.imageskip2.setOnClickListener {
            state.On = context?.getSharedPreferences("NewPrefences",Context.MODE_PRIVATE)!!.getBoolean("On1",false)
            state.On = context?.getSharedPreferences("NewPrefences", Context.MODE_PRIVATE)!!.getBoolean("On2",false)
            state.On = context?.getSharedPreferences("NewPrefences", Context.MODE_PRIVATE)!!.getBoolean("On3",false)
            welcomeView.checkPassed()
        }
        ifWePassed()
    }
